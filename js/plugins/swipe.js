;(function(M, doc, plugin){
    ...
    function generate( self, parent ){
        ...
        var s = self.$sets;
        var html = '<div class="swipe"><div class="swipe-area"><ul class="swipe-track" style="transform:translate3d(0,0,0);width:';
        ...
        /* PREV & NEXT */
        html += '<a class="swipe-arrow swipe-prev">'+ (s.prev || '&lt;') +'</a>\
                <a class="swipe-arrow swipe-next">'+ (s.next || '&gt;') +'</a></div>';

        /* Navs */
        html += '<ul class="swipe-navs">';
    ...
    function formatSlider( nodes, l ){
        ...
        for(; i < l; i++){
            ...
            if( el.tagName === 'LI' ){
                ...
                outer = '<li class="swiper" data-swipe="'+ i +'">'+ el.outerHTML +'</li>';
            }
            tpl[i] = outer;
        }
    ...
    function formatNavs( l ){
        var navs = '';
        while( l >= 0 ){
            navs = '<li data-swipe="'+ l +'">'+ l +'</li>' + navs;
            --l;
        }
        return navs;
    }
    ...
    function mount( self, el ){
        ...
        self.target = el;
        self.track = el.querySelector('.swipe-track');
        self.navs = s.navs? el.querySelector('.swipe-navs'): undefined;

        self.arrowPrev = el.querySelector('.swipe-prev');
        self.prev = function(){ return self.goto( self.current - s.scroll, false ); };
        self.arrowNext = el.querySelector('.swipe-next');
        self.next = function(){ return self.goto( self.current + s.scroll, true ); };

        self.goto = goto;
        self.destroy = destroy;

        self.swipe = function(){ return self; }
    }
    ...
    function goto( i, direct ){
        ...
        if( self.navs ){
            var nav = self.navs.querySelector('.swipe-active');
            nav && nav.classList.remove('swipe-active');
            var navs = self.navs.children;
            nav = navs[Math.ceil(i / s.scroll)];
            nav && nav.classList.add('swipe-active');
        }

        M('.swiper', self.track).each(function( swiper, j ){
            j += l * 2;
            swiper.classList[(j - i)%l < s.display ? 'add': 'remove']('swipe-active');
        });
    ...
    function destroy(){
        ...
        el.classList.remove('swipe-initialized');
        el.innerHTML = self.html;
        el.$swipe = self = self.target = null;
    }
    ...
    function initialize( self, el ){
        ...
        self.navs && self.navs.addEventListener(s.navsTrigger, function( e ){
            var nav = e.target;
            if( nav.matches('li') && !nav.classList.contains('swipe-active') ){
                self.goto(nav.dataset.swipe * s.scroll);
            }
        }, true);
        ...
        (!s.prev || f) && self.arrowPrev.classList.add('swipe-hidden');
        (!s.next || f) && self.arrowNext.classList.add('swipe-hidden');
        ...
        el.classList.add('swipe-initialized');
    }
    ...
    function reSlide( self ){
        ...
        if( s.navs ){
            if( f ){
                ...
                self.navs = self.target.querySelector('.swipe-navs');
                self.navs.innerHTML = formatNavs(Math.ceil((l - s.display) / s.scroll));
            }
        }

        self.rawSets.prev && self.arrowPrev.classList[f? 'add': 'remove']('swipe-hidden');
        self.rawSets.next && self.arrowNext.classList[f? 'add': 'remove']('swipe-hidden');
	...
	M.MASIV[plugin] = function( a, b ){
        return this.each(function( el ){
            var $swipe = el.$swipe;
            $swipe?
            (typeof $swipe[a] === 'function' && $swipe[a](b) ) :
            new Slide( el, a );
        });
	}
